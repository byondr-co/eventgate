...
const auditEtagCache = createEtagCache();

const fetcher = (url: string): Promise<AuditListResponse> =>
  auditEtagCache.fetchJSON<AuditListResponse>(url);
...
export function useAuditEvents(orgSlug: string, eventSlug: string, prefix: string) {
  ...
  return useQuery({
    ...
    queryFn: () => fetcher(`/api/v1/orgs/${orgSlug}/events/${eventSlug}/audit/${qs}`),
